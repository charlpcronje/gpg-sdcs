...
class Upload  {
     ...
     * @var array
     */
    protected array $fileTypesToEncrypt = [];
    ...
    /** Stores the failure uploads.
     * @var array
     */
    protected array $failureUploads = [];
    ...
    /**
     * Set the path directory where you want to upload the files(if not specified file/files
     ...
     */
    public function success(Closure $callback, bool $asObject = true)  {
     ...
     * @throws ReflectionException
     */
    public function error(Closure $callback, bool $asObject = true)  {
        $reflector = new ReflectionFunction($callback);
     ...
     * @return boolean
     */
    protected function shouldBeEncrypted(array $file): bool  {
     ...
     * @return boolean
     */
    protected function inOnlyArray($file): bool {
     ...
     * @return $this
     */
    public function encryptFileNames(bool $encrypt = false): Upload  {
        if (!$encrypt) {
            return;
        }
    ...
    public function encrypt(string $fileName): string {
        ...
        return @base64_encode(openssl_encrypt($fileName, $encryptMethod, static::KEY));
    }
     ...
     */
    public function create(bool $create = false) {
        if (!$create) {
            return;
        }
    ...
    public function unsuccessfulFilesHas(): bool {
        foreach ($this->files as $file) {
            if (!$file['success'] && ! empty($file['errorMessage'])) {
                return true;
            }
        }
        return false;
    ...
    /**
     * This method displays the errors formatted nicely with bootstraps.
     * @return void
     */
    ...
    protected function randomString($length = 64): string {
        $string = '';
    ...
    /**
     * A simple generator of a random key to use for encrypting.
     * @return void
     */
